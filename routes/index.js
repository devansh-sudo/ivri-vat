...
router.post("/create-doctor", async (req, res) => {
  const { fullName, email, address, mobno, expertise, password ,latitude ,longitude} = req.body;
  try {
...
router.get("/doctor/:lat/:long/:distance",async (req, res) => {
...
let where = {};
  ...
  try {
    const data = await Doctor.find(where);
